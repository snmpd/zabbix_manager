...
describe "ZabbixManager::Applications" do
  let(:actions_mock) { ZabbixManager::Applications.new(client) }
  let(:client) { double }
  ...
  describe ".create_or_update" do
    ...
    context "when an API request raise ApiError" do
      before do
        allow(actions_mock).to receive(:create).with(data).and_raise(ZabbixManager::ApiError, "ApiError occured.")
      end

      it "propogates the ApiError raise by an API" do
        expect { subject }.to raise_error(ZabbixManager::ApiError, "ApiError occured.")
      end
    end
